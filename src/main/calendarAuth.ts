...
export function setupMicrosoftCalendarAuth() {
  ipcMain.handle('connect-microsoft-calendar', async () => {
    try {
      const authWindow = new BrowserWindow({
        width: 800,
        height: 600,
...
export async function handleMicrosoftAuth(): Promise<AuthResponse> {
  try {
    // ... existing code ...
    return {
      success: false,
